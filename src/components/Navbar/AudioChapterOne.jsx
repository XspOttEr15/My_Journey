...
const AudioChapterOne = () => {
  const { ColseBgmusic,  RoomVolumn, setRoomVolumn } = useContext(DataContext);
  ...
  const handleVolumeChange = (event) => {
    setRoomVolumn(event.target.value);
  };

  return (
    <>
      <audio ref={audioRef} loop className="audio-control">
        <source src="/audios/bgm_room.mp3" type="audio/mpeg" />
      </audio>
...
export default AudioChapterOne;
